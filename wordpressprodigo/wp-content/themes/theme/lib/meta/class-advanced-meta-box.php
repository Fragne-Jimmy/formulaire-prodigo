 ...
 */
/*
 *Changes: 1.8:	-Added date-select, embed, taxonomy and parent
...
class Advanced_Meta_Box extends RW_Meta_Box {
	...
	function __construct($metabox) {
		...
		$this->check_field_parent();
	}
	function check_field_parent() {
		add_action('wp_ajax_rw_parent_link', array($this, 'get_parent_link'),5,1);	
	}
	...
	function get_parent_link() {
		...
		if(is_numeric($_POST['data']) && $_POST['data'] > 0){
			echo get_edit_post_link($_POST['data'], false);
		} else {
			die(1);
		}
		die(0);
	...
	function check_field_embed() {
		...
		add_action('wp_ajax_advanced_check_embedible', array(&$this, 'check_embedible'));	
	...
	function show() {
		...
		if($this->_meta_box['update']) {
			...
			echo '<a href="#" class="update_data button-secondary" rel = "'.$this->_meta_box['id'].'_update|',$nonce,'|',$post->ID,'|',$post->post_type,'">Update</a>';
			echo '<a href="#" class="clear_data button-secondary">Clear</a>';
			echo "</td></tr>";
		}
		echo '</table>';
	}
	...
	function get_field_name($field, $post_id) {
		$name = /*$field['id']*/ $this->_meta_box['id'] . "[" . $field['id'] . "]" . (($field['multiple'] || $field['type'] === 'taxonomy') ? "[]" : "");
		return $name;
	}
	...
	function show_field_taxonomy($field, $meta, $post_id='') {
		if (!is_array($meta)) $meta = (array) $meta;	
		if($field['options']['add_new']) echo "<div class='rw-terms'>";
		$this->walk_taxonomy($field,$meta,$post_id, false);
		if($field['options']['add_new']) echo "</div>";
		...
		if($field['options']['add_new']) {
			...
			echo "<label>Name: <input type='text' class='rw-new-term' name='rw-new-term[]' /></label><br />";
			...
			echo "<div style='margin-top: 10px; margin-bottom: 10px;'><a href='#' rel='{$action}|{$nonce}|{$post_id}|{$field['options']['taxonomy']}' class='button-secondary rw-add-new-term'>Add</a>";
			echo "<a href='#'  class='button-secondary rw-term-cancel'>Cancel</a>";
			echo "</div></div>";
			echo "<a href='#' class='rw-show-add-term'> Add Term </a>";
		}
	...
	function show_field_parent($field, $meta, $post_id='') {
		...
		$results = get_posts(array('post_type'=>$field['parent_type'] , 'posts_per_page'=>-1));
		echo '<select name="parent_id" id="parent_id" class="rw-parent ',$field['id'],'">';
		if($field['optional']) {
			echo '<option value = "0">None</option>';
		}
		foreach ($results as $r) {
			echo '<option value="', $r->ID, '"', $r->ID == $current_post->post_parent ? ' selected="selected"' : '', '>', $r->post_title, '</option>';
		}
		echo '</select>';
		if(is_numeric($current_post->post_parent) && $current_post->post_parent > 0) {
			echo "<a href = '".get_edit_post_link($current_post->post_parent)."' class ='button rw-parent-edit' target = '_blank' >Edit</a>";
		} else {
			echo "<a href = '#' class ='button rw-parent-edit disabled' target = '_blank' >Edit</a>";
		}
	}
	
	function show_field_date($field, $meta, $post_id = '') {
	...
	function show_field_time($field, $meta,  $post_id = '') {
	...
	function show_field_embed($field, $meta, $post_id='') {	
		...
		echo "<a  href='#' class='veda_remove_embed button hidden'>Hide Embed </a>";
	...
	function save($post_id) {
		...
		$data = $_POST[$this->_meta_box['id']];
		//New way to save data.  More recursive friendly
		$this->save_post_data($post_id,$this->_fields,$data);
	}
	function save_post_data($post_id, $fields, $data) {
		...
		foreach ($fields as $field) {
			...
			// Validate via filter			
			if(has_filter('rw_validate_field_'.$field['id'])) {
				$new = apply_filter('rw_validate_field_'.$field['id'], $new, $post_id);
			} elseif (has_filter('rw_validate_field')) {
				$new = apply_filter('rw_validate_field', $new, $post_id, $name);				
			}
			...
			if(isset($field['fields'])) {
				$this->save_post_data($post_id, $field['fields'], $data); 	
			}
		}
	...
	function save_field_embed($post_id, $field, $old, $new) {
		$this->save_field($post_id,$field,$old,$new);
		$embed = $this->get_embed_html($new);
		if(!$embed || $embed == 'false' ){
			delete_post_meta($post_id, $field['id'].'_embed');
		} else {
			update_post_meta($post_id, $field['id'].'_embed', $embed);
		}
	}
