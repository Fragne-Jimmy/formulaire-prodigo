...
jQuery(document).ready(function($) {
	//parent
	$('a.rw-parent-edit').live('click', function() {
		if($(this).hasClass('disabled')) {
			return false;	
		}
	});
	
	$('select.rw-parent').live('change', function() {
		data = $(this).val();
		parent_select = $(this);
		$.post(ajaxurl, {action: 'rw_parent_link', data: data}, function(response){
			if (response == "1") {
				parent_select.siblings('a.rw-parent-edit').addClass('disabled').attr('href','#');
			} else {
				parent_select.siblings('a.rw-parent-edit').removeClass('disabled').attr('href',response);
	...
	$(".veda_view_embed").live("click", function() {
		var data = $(this).siblings(".embed_url").val(),
			nonce = $(this).siblings(".embed_nonce").val(),
			container = $(this).siblings("div.veda_embed");
		$.post(ajaxurl, {action: 'advanced_show_embed', data: data, nonce: nonce}, function(response){
			if (response.indexOf('false') > 0 || response == 1) {
				alert( "Link Not Embeddible.");
			} else {
				container.html(response);	
				container.siblings(".veda_remove_embed").show();
			}
		});
		return false;
	});
	...
	$(".veda_remove_embed").live("click", function() {
		var	container = $(this).siblings(".veda_embed");
		container.empty();
		$(this).hide();
		return false;
	});
